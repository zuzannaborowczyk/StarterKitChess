...
public class KingMovesValidator implements MoveValidation {
	Board board;
	Color nextMoveColor;

	public KingMovesValidator(Board board) {
		this.board = board;
	}
	...
	public boolean isMovePosible(Coordinate from, Coordinate to) throws InvalidMoveException {
		...
		int xTo = to.getX();
		int yTo = to.getY();
		if (Math.abs(xTo - xFrom) == 1 && yFrom == yTo) {
			return true;
		} else if (Math.abs(yTo - yFrom) == 1 && xFrom == xTo) {
			return true;
		} else if (Math.abs(xTo - xFrom) == 1 && Math.abs(yTo - yFrom) == 1) {
			return true;
		} else {
